...
va_list vap)
{
    extern HSim__s6 * createworkM_f_u_l_l___s_u_b_t_r_a_c_t_o_r___s(const char*);
    HSim__s6 *blk = createworkM_f_u_l_l___s_u_b_t_r_a_c_t_o_r___s(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_full_subtractor_de_behavioral(const char*);
    HSim__s6 *blk = createWork_full_subtractor_de_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","FULL_SUBTRACTOR_S", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("full_subtractor_de/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("full_subtractor_de", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_f_u_l_l___s_u_b_t_r_a_c_t_o_r___s(const char*);
        topvl = (HSim__s5*)createworkM_f_u_l_l___s_u_b_t_r_a_c_t_o_r___s("FULL_SUBTRACTOR_S");
