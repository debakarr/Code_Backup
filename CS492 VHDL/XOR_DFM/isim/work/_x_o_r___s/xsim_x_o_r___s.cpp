...
va_list vap)
{
    extern HSim__s6 * createworkM_x_o_r___s(const char*);
    HSim__s6 *blk = createworkM_x_o_r___s(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_xor_de_behavioral(const char*);
    HSim__s6 *blk = createWork_xor_de_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","XOR_S", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("xor_de/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("xor_de", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_x_o_r___s(const char*);
        topvl = (HSim__s5*)createworkM_x_o_r___s("XOR_S");
