...
va_list vap)
{
    extern HSim__s6 * createworkM_a_n_d___s(const char*);
    HSim__s6 *blk = createworkM_a_n_d___s(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_and_de_behavioral(const char*);
    HSim__s6 *blk = createWork_and_de_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","AND_S", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("and_de/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("and_de", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_a_n_d___s(const char*);
        topvl = (HSim__s5*)createworkM_a_n_d___s("AND_S");
