...
va_list vap)
{
    extern HSim__s6 * createworkM_n_o_t___s(const char*);
    HSim__s6 *blk = createworkM_n_o_t___s(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_not_de_behavioral(const char*);
    HSim__s6 *blk = createWork_not_de_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","NOT_S", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("not_de/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("not_de", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_n_o_t___s(const char*);
        topvl = (HSim__s5*)createworkM_n_o_t___s("NOT_S");
